...
class AmbLocomoSmokeWorkflowTests(unittest.TestCase):
    def test_enables_only_bm25_and_puffo_bm25_q1_arms(self) -> None:
        workflow = WORKFLOW.read_text(encoding="utf-8")

        self.assertEqual(workflow.count("--memory bm25 --query-limit 1"), 1)
        self.assertEqual(workflow.count("--memory puffo-bm25 --query-limit 1"), 1)
        self.assertNotIn("--memory puffo --query-limit 1", workflow)
